import { useEffect, useState } from "react";
import style from "./Header.module.css";
import "./header.css";
import { CiSearch } from "react-icons/ci";
import logo from "../../img/HomeDecor.png";
import { Link, NavLink } from "react-router-dom";
import { SlBasket } from "react-icons/sl";
import { MdMenu } from "react-icons/md";
import { GoX } from "react-icons/go";
import Search from "../search/Search";
import { useSelector } from "react-redux";

const Header = () => {
  const { logined } = useSelector((state) => state.user);

  const [options, setOptions] = useState(false);
  const [activeOption, setActiveOption] = useState("AZ");
  const [searchIsOpen, setSearchIsOpen] = useState(true);

  const changeLang = (e) => {
    setActiveOption(e.target.textContent);
  };

  const selectOptions = () => {
    setOptions((prev) => !prev);
  };

  const openSearch = () => {
    setSearchIsOpen(!searchIsOpen);
  };

  return (
    <>
      <Search closeSearch={openSearch} close={searchIsOpen} />
      <header className={style.header}>
        <div className="container">
          <div className={style.headerBottom}>
            <div className={style.idiomas}>
              <div onClick={selectOptions} className={style.lang}>
                {activeOption}
                {options ? (
                  <ul>
                    <li onClick={changeLang} className="idiOption">
                      AZ
                    </li>
                    <li onClick={changeLang} className="idiOption">
                      EN
                    </li>
                    <li onClick={changeLang} className="idiOption">
                      ESP
                    </li>
                  </ul>
                ) : (
                  false
                )}
              </div>
              <div>
                <Link>
                  <button onClick={openSearch} className={style.search}>
                    <CiSearch />
                  </button>
                </Link>
              </div>
            </div>
            <div className={style.logo}>
              <img src={logo} alt="" />
            </div>
            <div className={style.icons}>
              <MdMenu className={style.MdMenu1} />{" "}
              <button className={style.search1}>
                <CiSearch />
              </button>
              <Link
                to="/shoppingCart"
                className={[style.basket, style.cart].join(" ")}
              >
                <div className={style.cart_box}>
                  <SlBasket /> <span className={style.badge}>1</span>
                </div>
              </Link>
              <Link
                to={logined ? `/myaccountlayout` : "/auth"}
                className={style.account}
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="19"
                  height="19"
                  viewBox="0 0 24 24"
                  fill="none"
                >
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M12 1.25C9.37665 1.25 7.25 3.37665 7.25 6C7.25 8.62335 9.37665 10.75 12 10.75C14.6234 10.75 16.75 8.62335 16.75 6C16.75 3.37665 14.6234 1.25 12 1.25ZM8.75 6C8.75 4.20507 10.2051 2.75 12 2.75C13.7949 2.75 15.25 4.20507 15.25 6C15.25 7.79493 13.7949 9.25 12 9.25C10.2051 9.25 8.75 7.79493 8.75 6Z"
                    fill="#2D2D2B"
                  />
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M12 12.25C9.96067 12.25 8.07752 12.7208 6.67815 13.5204C5.3 14.3079 4.25 15.5101 4.25 17C4.25 18.4899 5.3 19.6921 6.67815 20.4796C8.07752 21.2792 9.96067 21.75 12 21.75C14.0393 21.75 15.9225 21.2792 17.3219 20.4796C18.7 19.6921 19.75 18.4899 19.75 17C19.75 15.5101 18.7 14.3079 17.3219 13.5204C15.9225 12.7208 14.0393 12.25 12 12.25ZM5.75 17C5.75 16.2807 6.26701 15.483 7.42236 14.8228C8.55649 14.1747 10.1733 13.75 12 13.75C13.8267 13.75 15.4435 14.1747 16.5776 14.8228C17.733 15.483 18.25 16.2807 18.25 17C18.25 17.7193 17.733 18.517 16.5776 19.1772C15.4435 19.8253 13.8267 20.25 12 20.25C10.1733 20.25 8.55649 19.8253 7.42236 19.1772C6.26701 18.517 5.75 17.7193 5.75 17Z"
                    fill="#2D2D2B"
                  />
                </svg>
              </Link>
            </div>
          </div>
          <nav className={style.nav}>
            <ul className={style.list}>
              <li>
                <NavLink to="/">Home</NavLink>
              </li>
              <li>
                <NavLink to="/about">About</NavLink>
              </li>
              <li>
                <NavLink to="/products">Products</NavLink>
              </li>
              <li>
                <NavLink to="/collections">Collections</NavLink>
              </li>
              <li>
                <NavLink to="/contactpage">Contact</NavLink>
              </li>
            </ul>
          </nav>
        </div>
        <div className={style.mobileMenu}>
          <div className="container">
            <div className={style.mobileActions}>
              <div className={style.cartUser}>
                <button>
                  <Link
                    to="/shoppingCart"
                    className={[style.basket, style.mobile_cart].join(" ")}
                  >
                    <div className={style.cart_box}>
                      <SlBasket /> <span className={style.badge}>1</span>
                    </div>
                  </Link>
                </button>
                <button>
                  <Link to="/myaccountlayout" className={style.account_mobile}>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="19"
                      height="19"
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <path
                        fillRule="evenodd"
                        clipRule="evenodd"
                        d="M12 1.25C9.37665 1.25 7.25 3.37665 7.25 6C7.25 8.62335 9.37665 10.75 12 10.75C14.6234 10.75 16.75 8.62335 16.75 6C16.75 3.37665 14.6234 1.25 12 1.25ZM8.75 6C8.75 4.20507 10.2051 2.75 12 2.75C13.7949 2.75 15.25 4.20507 15.25 6C15.25 7.79493 13.7949 9.25 12 9.25C10.2051 9.25 8.75 7.79493 8.75 6Z"
                        fill="#2D2D2B"
                      />
                      <path
                        fillRule="evenodd"
                        clipRule="evenodd"
                        d="M12 12.25C9.96067 12.25 8.07752 12.7208 6.67815 13.5204C5.3 14.3079 4.25 15.5101 4.25 17C4.25 18.4899 5.3 19.6921 6.67815 20.4796C8.07752 21.2792 9.96067 21.75 12 21.75C14.0393 21.75 15.9225 21.2792 17.3219 20.4796C18.7 19.6921 19.75 18.4899 19.75 17C19.75 15.5101 18.7 14.3079 17.3219 13.5204C15.9225 12.7208 14.0393 12.25 12 12.25ZM5.75 17C5.75 16.2807 6.26701 15.483 7.42236 14.8228C8.55649 14.1747 10.1733 13.75 12 13.75C13.8267 13.75 15.4435 14.1747 16.5776 14.8228C17.733 15.483 18.25 16.2807 18.25 17C18.25 17.7193 17.733 18.517 16.5776 19.1772C15.4435 19.8253 13.8267 20.25 12 20.25C10.1733 20.25 8.55649 19.8253 7.42236 19.1772C6.26701 18.517 5.75 17.7193 5.75 17Z"
                        fill="#2D2D2B"
                      />
                    </svg>
                  </Link>
                </button>
              </div>
              <button className={style.close_btn}>
                <GoX />
              </button>
            </div>
            <div className={style.mobile_nav}>
              <ul>
                <li>
                  <NavLink to="/" className={style.active}>
                    Home
                  </NavLink>
                </li>
                <li>
                  <NavLink to="/about">About</NavLink>
                </li>
                <li>
                  <NavLink to="/products">Products</NavLink>
                </li>
                <li>
                  <NavLink to="/collections">Collections</NavLink>
                </li>
                <li>
                  <NavLink to="/contactpage">Contact</NavLink>
                </li>
              </ul>
            </div>
            <div className={style.langs}>
              <ul>
                <li
                  onClick={changeLang}
                  className={[(style.lang, style.active)].join(" ")}
                >
                  AZ
                </li>
                <li onClick={changeLang} className={[style.lang].join(" ")}>
                  EN
                </li>
                <li onClick={changeLang} className={[style.lang].join(" ")}>
                  ESP
                </li>
              </ul>
            </div>
          </div>
        </div>
      </header>
    </>
  );
};

export default Header;
